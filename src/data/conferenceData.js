import iranaImg from "../../Dr.Irana_M_Hittalamani.jpeg";
import rajendraImg from "../../Dr.Rajendra_Pratap_Gupta.jpeg";
import umashankarImg from "../../Dr.Umashankar Subramnyam.jpeg";
import mujeebImg from "../../Mr.Mujeeb_C_kandi.jpeg";
import umaNambiarImg from "../../Dr.Uma_Nambiar.jpeg";


export const conferenceData = {
  title: "DIGITAL HEALTH DAY 2026",
  organizers: [
    "KLE Academy of Higher Education and Research",
    "JNMC School of Allied Health Sciences",
    "Department of Digital Health & Health Information Management",
    "Jawaharlal Nehru Medical College, Belagavi",
  ],
  date: "12 May 2026",
  venue: "KLE Centenary Convention Center, JNMC Campus, Belagavi",
  navItems: [
    { id: "hero", label: "Home" },
    { id: "about", label: "About" },
    { id: "speakers", label: "Chief Guest & Speakers" },
    { id: "committee", label: "Committee" },
    { id: "schedule", label: "Schedule" },
    { id: "guidelines", label: "Competition Guidelines" },
    { id: "registration", label: "Registration" },
    { id: "contact", label: "Contact" },
  ],
  departmentText: [
    "The Department of Digital Health and Health Information Management is an academic unit under the School of Allied Health Sciences, dedicated to education, research, and capacity building in digital health and health information management.",
    "It focuses on training students in health informatics, electronic health records, and systematic management of health information for effective healthcare delivery. The department integrates healthcare, information technology, and data science to support evidence-based practice.",
    "It offers academic programs and practical learning experiences that prepare students for roles in modern healthcare environments and contributes to the advancement of digital health aligned with National Digital Health Mission.",
  ],
  eventText: [
    "Digital Health Day 2026 is an academic event aimed at promoting awareness and understanding of digital transformation in healthcare.",
    "The program features expert sessions on digital health transformation, telemedicine implementation, health data quality, and electronic health record adoption.",
    "It provides a platform for knowledge sharing among students, faculty, and healthcare professionals, along with interactive activities such as a health quiz and e-poster competition.",
    "The event aims to bridge skill gaps and prepare professionals for evolving roles in the digital healthcare ecosystem.",
  ],
  themes: [
    "Digital Health Transformation",
    "Telemedicine & Healthcare Accessibility",
    "Health Data & Intelligence",
    "EHR Adoption",
    "Digital Health Innovation & Research",
    "Student Engagement & Knowledge Enhancement",
  ],
  dignitaries: [
    {
      name: "Dr. Uma Nambiar",
      designation: "CEO, IISc Medical School Foundation | Co-Founder & Chairperson, DHIA | Vice President, Telemedicine Society of India | Independent Director & Board Member, Hemas Group of Hospitals, Sri Lanka",
      image: umaNambiarImg,
      description: "A trained neurosurgeon and healthcare administrator with over forty years in the healthcare industry. An alumnus of Armed Forces Medical College Pune, Sree Chitra Tirunal Institute Trivandrum, and Faculty of Management Studies at Delhi, with international exposure at Beth Israel Institute for Neurology and Neurosurgery, New York and Necker Enfant Malade, Paris.\n\nShe has worked across different hospitals in India and Africa as a Surgical Neuro-oncologist, as well as in CXO roles in leading corporate hospitals in India. She has commissioned greenfield hospitals and re-engineered brownfield hospitals across many geographies in India and abroad.\n\nShe has been a Special Technical Advisor to the Ministry of Health in Djibouti, East Africa and a consultant in Digital Health to a private hospital group in Botswana.\n\nShe is an independent board director for Hemas Hospital Group in Sri Lanka and IIHMR (Indian Institute of Health Management Research) in Bangalore. She is engaged in the capacity of board director, advisor, and mentor to multiple healthcare organizations including healthcare enterprises as well as health-tech startups.\n\nHer passion lies in the use of Digital Technology and mentoring students from across the healthcare segment. She is also an honorary professor at Jawaharlal Nehru Centre for Advanced Scientific Research.",
    },
  ],
  committee: {
    chiefPatrons: [
      { name: "Shri.Amit P.Kore", designation: "Hon'ble Chairman KLE Society" },
      { name: "Dr.Prabhakar Kore", designation: "Chancellor KAHER" }
    ],
    patrons: [
      { name: "Dr. Sandeep Shrivastava", designation: "Hon’ble Vice Chancellor, KAHER" }
    ],
    advisoryCouncil: [
      // Row 1
      { name: "Dr. Vishwanath M.Pattanshetti", designation: "Registrar, KAHER" },
      { name: "Dr. Rajesh S.Powar", designation: "Principal, JNMC, KAHER" },
      { name: "Dr. Chandra S.Metgud", designation: "COE, KAHER" },
      // Row 2
      { name: "Dr. Urmila Kagal", designation: "Director, SAHS KAHER" },
      { name: "Dr. Mubashir Angolkar", designation: "Prof. & Head School of Public Health, JNMC, KAHER" },
      { name: "Dr. Ramesh Paranjape", designation: "Distinguished Professor and Director BSRC KAHER" },
      // Row 3
      { name: "Dr. Uma M.H", designation: "Officer on Special Duty-KAHER" },
      null,
      null
    ],
    organizingCommittee: [
      { title: "Organizing Chairman:", name: "Dr. Col. M. Dayananda", designation: "Medical Director, KLE's Prabhakar Kore Hospital and MRC" },
      { title: "Organizing Sectretary :", name: "Dr.Deepa Nair", designation: "Associate Professor & Head of Dept.of Digital Health - JNMC,KAHER" }
    ],
    members: [
      // Row 1
      { name: "Dr. Nanda Kumar G.", designation: "Principal, KLE HMCT" },
      { name: "Dr. Mayank Singh", designation: "Asst.Prof. Biostatistics-KAHER" },
      { name: "Dr. Shivanand Tanwashi", designation: "Senior Resident,Forensic Medicine-JNMC" },
      // Row 2
      { name: "Dr. Annapurna Kari", designation: "Lecturer, School of Public Health JNMC" },
      { name: "Dr. Sushma Meled", designation: "Asst.Prof. Nutrition & Dietetics -JNMC" },
      { name: "Ms. Padmashree R", designation: "Tutor, Forensic Medicine JNMC" },
      // Row 3
      { name: "Mr. Pranesh K", designation: "IT Adminstrator -KAHER" },
      { name: "Mr. Sachin Vastrad", designation: "IT Technical Assistant - KAHER" },
      { name: "Mr. Alex M Carvalho", designation: "Lecturer, Dept. of Biostatistics - KAHER" }
    ]
  },
  speakers: [
    {
      name: "Dr. Rajendra Pratap Gupta",
      designation: "Keynote Speaker",
      topic: "Advancing Healthcare Systems Through Digital Transformation: Policy, Innovation and Global Perspectives",
      image: rajendraImg,
      isKeynote: true,
      description: "Dr. Rajendra Pratap Gupta is a leading global voice in Health and has done pioneering work in Health, working with various multi-lateral bodies and governments. He has been interviewed by Harvard Business Review and has been featured in the world’s Top 50 Thinkers Podcast by Deloitte & Thinkers 50, and was conferred the Global Impact Award by Personal Connected Health Alliance (HIMSS) in 2018 in Boston. As an AI researcher, he is the creator of the Functional AI Pyramid - AI Maturity Model.\n\nHis book ‘Digital Health - Truly Transformational’ remains the global bestseller and is the most comprehensive book on Digital Health ever written. He is also a public policy expert with vast experience in policymaking across sectors, and he was instrumental in drafting the election manifesto (all sectors) for the BJP in 2014. He has played a key role in drafting the National Health Policy-2017, National Digital Health Mission, the National Education Policy-2020, and the Viksit Bharat Abhiyan (Developed India Mission) - which is the flagship program of the Modi Government for New India.\n\nHe has been invited by all multi-lateral bodies: The United Nations, ITU, FAO, WHO, World Bank, Asian Development Bank, World Economic Forum, and the Commonwealth. Besides, he has been invited by governments of the United States of America, the United Kingdom, Govt. of Japan, Algeria, Finland, Norway, Ethiopia & Bangladesh, and also by the Ministry of Health, Ministry of Human Resource Development, Ministry of Labour and Employment & The Planning Commission – Government of India, for his views on a diverse range of topics.\n\nDr. Rajendra has been involved in major global initiatives and holds several key positions in the UN bodies. He leads three dynamic coalitions at the United Nations’ Internet Governance Forum: DC on Digital Economy, Environment, and Digital Health, and Chairman - Commonwealth AI Consortium (Capacity Building).\n\nHe has been a co-author of several reports at the World Health Organisation (WHO); served on the guidelines development group for Digital Health at the WHO; invited by the Prime Minister’s office of Bangladesh to help draft the AI Policy. He is the former advisor to the Union Health Minister, Government of India. He was nominated by the World Economic Forum on the Global Agenda Council for Digital Health. He has authored five books on a diverse range of topics and delivered over 300 talks around the world.\n\nHe is the chairman of the Digital First Foundation, the Co-chair of the Global Commission for 21st Century Healthcare, chairman of the Academy of Digital Health Sciences and the Consortium of Innovative Healthcare Universities (CIHU) and also, chairs the Medical Education Summit held on the sidelines of the Global Digital Health Summit every year.",
    },
    {
      name: "Dr. Uma Nambiar",
      designation: "Keynote Speaker",
      topic: "Preparing the Healthcare Workforce for the Digital Era",
      image: umaNambiarImg,
      isKeynote: true,
      description: "A trained neurosurgeon and healthcare administrator with over forty years in the healthcare industry. An alumnus of Armed Forces Medical College Pune, Sree Chitra Tirunal Institute Trivandrum, and Faculty of Management Studies at Delhi, with international exposure at Beth Israel Institute for Neurology and Neurosurgery, New York and Necker Enfant Malade, Paris.\n\nShe has worked across different hospitals in India and Africa as a Surgical Neuro-oncologist, as well as in CXO roles in leading corporate hospitals in India. She has commissioned greenfield hospitals and re-engineered brownfield hospitals across many geographies in India and abroad.\n\nShe has been a Special Technical Advisor to the Ministry of Health in Djibouti, East Africa and a consultant in Digital Health to a private hospital group in Botswana.\n\nShe is an independent board director for Hemas Hospital Group in Sri Lanka and IIHMR (Indian Institute of Health Management Research) in Bangalore. She is engaged in the capacity of board director, advisor, and mentor to multiple healthcare organizations including healthcare enterprises as well as health-tech startups.\n\nHer passion lies in the use of Digital Technology and mentoring students from across the healthcare segment. She is also an honorary professor at Jawaharlal Nehru Centre for Advanced Scientific Research.",
    },
    {
      name: "Dr. Umashankar Subramanian",
      designation: "Speaker",
      topic: "Healthcare without Walls: The Telemedicine Revolution",
      image: umashankarImg,
      description: "Prof (Dr) Umashankar Subramanian is a distinguished academician, entrepreneur, administrator, and mentor specializing in public health informatics and global health. He holds a BHMS degree from Rajiv Gandhi University of Health Sciences, Bangalore, and a Master of Public Health from the University of South Florida, USA.\n\nWith over 22 years of experience in clinical practice and public health, he has made significant contributions to digital health, public health informatics, and healthcare innovation. His work spans major public health domains including HIV/AIDS, tuberculosis, cancer prevention, road safety, sustainability, and solid waste management. He has also led the development of healthcare technology solutions, including a COVID-19 screening application and other digital health platforms.\n\nHe is currently the Co-Founder and Managing Director of Arogyayati Private Limited (Med.Bot), and serves as the Honorary Secretary of the Telemedicine Society of India. He is also an Adjunct Professor at Dayanand Sagar University and Co-founder of Niravadhi Edu Foundation.\n\nPreviously, he served as Director of International Programs and Public Health at Meenakshi Mission Hospital and Research Centre, Madurai, where he established one of India’s most impactful telemedicine programs, recognized by Dr. APJ Abdul Kalam and included in the PURA initiative. He has also held leadership roles including Principal of Padmashree School of Public Health and Director at LifeRing Technologies Ltd.\n\nDr. Umashankar actively contributes to academia and policy through his roles as a subject expert for the National Commission for Homoeopathy (Ministry of AYUSH) and as a member of multiple Boards of Studies and Academic Advisory Councils across reputed institutions. He has also been involved in curriculum development for public health and homoeopathy programs.\n\nHe plays key roles in ethics and governance, serving as Chairman and member of various Institutional Ethics Committees. Additionally, he acts as an advisor to multiple organizations in healthcare, education, and technology sectors.\n\nA recognized thought leader, he has presented and published extensively in national and international forums. His multidisciplinary contributions continue to influence digital health transformation, public health systems, and healthcare education in India.",
    },
    {
      name: "Mr. Mujeeb C Kandi",
      designation: "Speaker",
      topic: "From Health Records to Health Intelligence:- Data Quality in the Real World",
      image: mujeebImg,
      isVirtual: true,
      description: "Mujeeb C. Kandy is a health information management professional with over 20 years of experience driving HIM transformations and data intelligence initiatives in healthcare. He holds a Master of Science in Health Information Management (USA), a Master of Applied Science in Medical Documentation (India), and certifications including RHIA, CPHQ, HL7, and a Postgraduate Certificate in Machine Learning and Artificial Intelligence.\n\nMujeeb actively integrates AI and advanced analytics into healthcare operations, focusing on predictive modeling, centralized health intelligence, clinical documentation improvement, and data governance. His work bridges artificial intelligence, health information standards, and practical implementation within large healthcare systems.\n\nAn elected Regional Director of IFHIMA, he contributes to global education, research, and AI focused discussions, and has spoken at international forums including AHIMA, AAPC Healthcon, and IMIA MedInfo. He is also a founding Vice President of Indian Association of Health Information Management and Health Informatics. He is passionate about responsible AI adoption, interoperability, and data-driven leadership, Mujeeb advocates for safe, ethical, and scalable AI solutions that enhance care quality, operational efficiency, and patient centered outcomes.",
    },
    {
      name: "Dr. Iranna M Hittalamani",
      designation: "Speaker",
      topic: "EHR Adoption: Clinician's Perspective",
      image: iranaImg,
      description: "Dr. Iranna M Hittalamani is an accomplished Interventional Radiologist, academician, and mentor with specialized expertise in hepatobiliary, transplant, and oncological imaging and interventions. He holds an MBBS from Rajiv Gandhi University of Health Sciences, Belagavi, an MD in Radiodiagnosis from BLDE University, Vijayapur, and a Fellowship in Endovascular and Interventional Radiology from Dr. Rela Institute & Medical Centre, Chennai.\n\nWith extensive clinical and academic experience, he is currently serving as an Assistant Professor in the Department of Interventional Radiology at Jawaharlal Nehru Medical College, Belagavi. His professional journey spans roles as Junior Resident, Senior Resident, and Fellow across reputed medical institutions, providing him with strong expertise in both diagnostic and interventional radiology practices.\n\nDr. Iranna has advanced competencies in conventional radiography, ultrasonography, CT and MRI reporting, and a wide spectrum of image-guided vascular and non-vascular interventions. His clinical interests include elastography and its translational applications, particularly in liver disease and fibrosis assessment.\n\nA passionate educator and mentor, he actively trains DM (Interventional Radiology) residents, MD (Radiology) postgraduates, and undergraduate medical students. He is the Founder and Mentor of the Interventional Radiology Interest Group for medical students at his institution, fostering early exposure to this specialized field.",
    },
  ],
  schedule: [
    ["9:00AM - 10:00AM", "Registration and Seating"],
    ["9:30AM - 9:45AM", "Thematic Skit on Digital Health Transformation"],
    ["9:45AM - 10:20AM", "<strong>Dr.Rajendra Pratap Gupta</strong><br/><strong>Keynote Address:</strong> Advancing Healthcare Systems Through Digital<br/>- Transformation: Policy, Innovation and Global Perspectives"],
    ["10:30AM - 11:30AM", "Inaugural Session"],
    ["11:30AM - 12:20PM", "<strong>Dr. Uma Nambiar</strong><br/><strong>Keynote Address:</strong> <em>Preparing the Healthcare Workforce for the<br/>- Digital Era</em>"],
    ["12:20PM - 1:00PM", "<strong>Dr. Umashankar Subrahmanyam</strong><br/><strong>Talk on :</strong> <em>Healthcare without Walls: The Telemedicine Revolution</em>"],
    ["1:00PM - 2:00PM", "Lunch Break"],
    ["2:00PM - 2:40PM", "<strong>Mr. Mujeeb C. Kandy</strong><br/><strong>Talk on:</strong> <em>From Health Records to Health Intelligence :-<br/>Data Quality in the Real World</em>"],
    ["2:45PM - 3:15PM", "<strong>Dr. Iranna M Hittalamani</strong><br/><strong>Talk on:</strong> <em>EHR Adoption: Clinician's Perspective</em>"],
    ["3:20PM - 4:20PM", "Health Quiz Competition"],
    ["4:30PM - 5:00PM", "Valedictory Session, Awards & Certificate Distribution"],
  ],
  parallelSession: [
    ["11:30 AM - 3:30 PM", "E- Poster Presentation"],
  ],
  registration: {
    fees: [
      { type: "Working Professionals", amount: "700" },
      { type: "Students", amount: "400" },
    ],
    note: "Registration is mandatory to participate in the conference.",
  },
  contact: {
    department: "Department of Digital Health & Health Information Management",
    institution: "JNMC, Belagavi",
    email: "digihealth.kaher@gmail.com",
    phone: "+91 6361344364",
  },
  guidelines: [
    {
      title: "Abstract Submission Guidelines for E-Poster",
      file: "/docs/E_Poster_Abstract_Guidelines.pdf",
    },
    {
      title: "Guidelines for Submission of Final E-Poster",
      file: "/docs/Final_E_Poster_Guidelines.pdf",
    },
    {
      title: "Health Quiz Competition Guidelines",
      file: "/docs/Health_Quiz_Competition_Guidelines.pdf",
    },
  ],
};
